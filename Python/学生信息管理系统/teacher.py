...
class teacher(wx.App):
    ...
    #主界面
    def frame(self):
        self.teacherFrame = wx.Frame(None, title=u"学生信息管理系统", size=(700, 500), pos=(300, 200))
        ...
        self.menu2 = wx.Menu()
        self.item3 = wx.MenuItem(self.menu2, wx.ID_ANY, u"修改课程和成绩", wx.EmptyString, wx.ITEM_NORMAL)
        self.menu2.Append(self.item3)
        self.menubar.Append(self.menu2, u"修改课程和成绩")
        ...
        self.item2 = wx.MenuItem(self.menu1, wx.ID_ANY, u"手动输入学生和课程信息", wx.EmptyString, wx.ITEM_NORMAL)
    ...
    def add(self,event):
        ...
        try:
            ...
            if self.pd(a):
                ...
                f4 = float(self.course4Text.GetValue())
                f5 = float(self.course5Text.GetValue())
                f6 = float(self.course6Text.GetValue())
                ...
                if f > 0:
                    g = g + f
                sql = "UPDATE grade SET course1=%.2f,course2=%.2f,course3=%.2f,sum=%.2f ,course_id=%.2f ,ccourse_mark=%.2f ccourse_time=%.2f WHERE num= %s;" % (d, e, f, g,f4,f5,f6,a)
                cr.execute(sql)
                db.commit()
            else:
                ...
                if f > 0:
                    g = g + f
                data = (a, b, c, d, e, f, g,f4,f5,f6)
                sql = "INSERT INTO grade (num, name, class,course1,course2,course3,sum,course_id ,ccourse_mark, ccourse_time) VALUES ( '%s', '%s','%s', %.2f,%.2f,%.2f,%.2f,'%s','%s','%s');" % data
                cr.execute(sql)
                db.commit()
                if not self.pd(a):
                    data = (a, b)
                ...
                self.course4Text.SetValue("")
                self.course5Text.SetValue("")
                self.course6Text.SetValue("")
    ...
    def Select(self,a, b):
        ...
        cr.execute(sql)
        res = cr.fetchone()#返回单个元组
        if res == None:
            return -1
        return res[0]
    #手动导入成绩
    def head(self, event):
        self.headFrame = wx.Frame(None, title="手动添加", size=(350, 600), pos=(500, 100))#标题
        self.headFrame.SetMaxSize((350,300))
        self.headFrame.SetMinSize((350, 300))
        self.headPanel = wx.Panel(self.headFrame, -1, size=(350, 300))#上面的面板
        ...
        # 设置图片在panel中的位置
        self.staticbmp = wx.StaticBitmap(self.headPanel, -1, img, pos=(-5, 300))
        # 调用函数设置图片的大小
        ...
        # 设置图片在panel中的位置
        self.staticbmp = wx.StaticBitmap(self.headPanel, -1, img, pos=(262, 300))
        # 调用函数设置图片的大小
        ...
        self.course4Label = wx.StaticText(self.headPanel, -1, "课程ID", pos=(30, 210))
        self.course5Label = wx.StaticText(self.headPanel, -1, "课程学分", pos=(30, 240))
        self.course6Label = wx.StaticText(self.headPanel, -1, "开课时间", pos=(30, 270))

        self.addButton = wx.Button(self.headPanel, -1, "添  加", size=(50, 25), pos=(130, 300))
        ...
        self.course4Text = wx.TextCtrl(self.headPanel, -1, pos=(120, 210), size=(150, 20))
        self.course5Text = wx.TextCtrl(self.headPanel, -1, pos=(120, 240), size=(150, 20))
        self.course6Text = wx.TextCtrl(self.headPanel, -1, pos=(120, 270), size=(150, 20))
        ...
        if self.rbox.GetStringSelection()=='学       号':
            ...
            try:
                ...
                if self.pd2(a):
                    ...
                    if float(self.course3Text.GetValue()) < 0:
                        ...
                        f = float(self.course3Text.GetValue())
                    if float(self.course4Text.GetValue()) < 0:
                        f4=self.Select(a,"course_id")
                    else:
                        f4 = float(self.course5Text.GetValue())
                    if float(self.course3Text.GetValue()) < 0:
                        f5=self.Select(a,"ccourse_mark")
                    else:
                        f5 = float(self.course6Text.GetValue())
                    if float(self.course3Text.GetValue()) < 0:
                        f6=self.Select(a,"ccourse_time")
                    else:
                        f6 = float(self.course3Text.GetValue())
                    g = 0
                    if d > 0:
                        g = g + d
                    if e > 0:
            ...
            try:
                ...
                if na == 1:
                    ...
                    db = connect()
                    cr = db.cursor()
                    sql = "UPDATE grade SET course1=%.2f,course2=%.2f,course3=%.2f,sum=%.2f ,course_id=%.2f ,ccourse_mark=%.2f, ccourse_time=%.2f WHERE num= %s';" % (
                        d, e, f, g, f4,f5,f6,a)
                    cr.execute(sql)
                    db.commit()
                    wx.MessageBox("修改成功！", '修改成功', wx.OK | wx.ICON_INFORMATION)
    ...
    #界面
    def numAlter(self,event):
        self.numAlterFrame = wx.Frame(None, title="修改成绩", size=(450, 400), pos=(500, 120))
        ...
        # 设置图片在panel中的位置
        self.staticbmp = wx.StaticBitmap(self.numAlterPanel, -1, img, pos=(0, 300))
        # 调用函数设置图片的大小
        ...
        self.course4Label = wx.StaticText(self.numAlterPanel, -1, "课程ID", pos=(145, 150))
        self.course4Text = wx.SpinCtrlDouble(self.numAlterPanel, -1, pos=(230, 150), size=(150, 20))
        self.course4Text.SetRange(-1, 100)
        self.course4Text.SetValue(-1)
        self.course4Text.SetIncrement(1)


        self.course5Label = wx.StaticText(self.numAlterPanel, -1, "课程学分", pos=(145, 180))
        self.course5Text = wx.SpinCtrlDouble(self.numAlterPanel, -1, pos=(230, 180), size=(150, 20))
        self.course5Text.SetRange(-1, 100)
        self.course5Text.SetValue(-1)
        self.course5Text.SetIncrement(1)


        self.course6Label = wx.StaticText(self.numAlterPanel, -1, "开课时间", pos=(145, 210))
        self.course6Text = wx.SpinCtrlDouble(self.numAlterPanel, -1, pos=(230, 210), size=(150, 20))
        self.course6Text.SetRange(-1, 100)
        self.course6Text.SetValue(-1)
        self.course6Text.SetIncrement(1)


        self.alterButton = wx.Button(self.numAlterPanel, -1, "修  改", size=(50, 25), pos=(245, 250))
