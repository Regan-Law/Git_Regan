...
class Login(wx.Frame):
    ...
    def frame(self):
        self.loginFrame = wx.Frame(None, title="学生信息管理系统", size=(340, 240), pos=(500, 300))
        ...
        lblList = ['管理员', '教师', '学生']
        self.rbox = wx.RadioBox(self.loginPanel, label='选择角色', pos=(60, 20), choices=lblList, majorDimension=1,
...
def connect():#连接数据库
    db = pymysql.connect(host='localhost', port=3306, user='root', passwd='yan011017', db='python', charset='utf8')#密码记得改
    return db

def createdb():#如果不存在数据库则创建
