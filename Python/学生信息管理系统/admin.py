...
class admin(wx.App):
    ...
    def alterPass(self, event):
        ...
        self.staticbmp.SetBitmap(self.resizebmp(myimg, 95, 90))
        lblList = ['管理员', '教师', '学生']
        self.rbox = wx.RadioBox(self.alterPassPanel, label='选择角色', pos=(5, 55), choices=lblList, majorDimension=1,
    ...
    def saveDb(self, event):
        ...
        if dialog.ShowModal() == wx.ID_OK:
            ...
            res = subprocess.run(
                "cd C:\\Program Files\\MySQL\\MySQL Server 8.0\\bin & mysqldump -u root -p%s python > %s" % (key, path),
